...
* {star_network}[Star network]: all nodes are connected to a special central node. This is the typical layout found in a {wireless_lan}[Wireless LAN], where each wireless client connects to the central {Wireless_access_point}[Wireless access point].
...
While the role of ATM is diminishing in favor of {next_generation_network}[next-generation networks], it still plays a role in the {last_mile}[last mile], which is the connection between an {internet_service_provider}[Internet service provider] and the home user. 
...
|Application layer
|{border_gateway_protocol}[BGP](iBGP, eBGP) {dynamic_host_configuration_protocol}[DHCP](v6) {domain_name_system}[DNS] EIGRP {file_transfer_protocol}[FTP] {hypertext_transfer_protocol}[HTTP] {https}[HTTPS] {internet_message_access_protocol}[IMAP] {lightweight_directory_access_protocol}[LDAP] IS-IS MGCP {mqtt}[MQTT] NNTP {network_time_protocol}[NTP] OSPF {post_office_protocol}[POP] PTP ONC/RPC RTP RTSP {routing_information_protocol}[RIP] SIP {simple_mail_transfer_protocol}[SMTP] {simple_network_management_protocol}[SNMP] {secure_shell}[SSH] Telnet {transport_layer_security}[TLS/SSL] XMPP
